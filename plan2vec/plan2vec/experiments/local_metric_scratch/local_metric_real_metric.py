"""
Training a local metric function to predict:

- 0 if points are identical
- 1 if points are in the neighborhood
- 2 if points are far-apart
...
class L2LocalMetric(nn.Module):
    """Returns 1 if inputs are neighbors, 0 otherwise
    """

    def __init__(self, input_dim, latent_dim, order=2):
        super().__init__()
        self.order = order
    ...
    def forward(self, x, x_prime):
        _ = self.trunk(x) - self.trunk(x_prime)
        return torch.norm(_, p=self.order, dim=-1)
...
def train(memory: ReplayBuffer, metric_fn, eval_only=False):
    ...
    loader = DataLoader(TripletLossDataset(all_x[shuffle][:valid_index], all_y[shuffle][:valid_index]),
                        batch_size=Args.batch_size, shuffle=True, num_workers=4)  # prevent debugger bugger error
    eval_loader = DataLoader(TripletLossDataset(all_x[shuffle][valid_index:], all_y[shuffle][valid_index:]),
                             batch_size=valid_index, shuffle=True, num_workers=4)  # prevent debugger bugger error
    ...
    def evaluate():
        with torch.no_grad():
            for i, (_, y) in enumerate(eval_loader):
                ...
                y_hat_int = np.where(y_hat < 1.5, 1, 2)
                correct = (y_hat_int == y.byte().numpy()).sum().item()
...
def main(freeze_after=True, **kwargs):
    ...
    f_local_metric = L2LocalMetric(2, 10).to(Args.device)
