...
def common_config():
    ...
    Args.load_top_k = "top_24-08.pkl"
    # Args.load_pairwise_ds = "~/"
...
def plan2vec(**kwargs):
    ...
    Args.update(kwargs)
    _ = instr(main, __postfix="plan2vec", **vars(Args), _DEBUG=vars(DEBUG))
    jaynes.run(_)
...
def plan2vec_binary_reward(**kwargs):
    ...
    Args.update(kwargs)
    _ = instr(main, __postfix="plan2vec_binary_reward", **vars(Args), _DEBUG=vars(DEBUG))
    jaynes.run(_)
...
if __name__ == "__main__":
    ...
    for Args.r_scale in [1 / 20., 1 / 40., 1 / 10., 5, 20]:
        Args.lr = 1e-3
