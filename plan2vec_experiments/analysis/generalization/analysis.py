...
@cli_parse
class Config:
    sweep_root = "geyang/plan2vec/2019/07-12/analysis/model-architectrure/model_architecture/binary_reward"
...
def collect_data(no_cache=False, include=None, exclude=("run*", "kwargs*", "host*"), predicate=None,
    ...
    with logger.PrefixContext(Config.sweep_root):
        ...
        for run_prefix, params in tqdm(df.iterrows(), total=len(df.index), desc="loading parameter"):
            _ = single_run_stats("/" + run_prefix, "epoch", "success_rate/mean", x_bin_size=100)
            if _ is None:
                continue
            idx = _['mean'].idxmax()
...
def make_bar_chart():
    ...
    df.set_index('Args.env_id')
    df.set_index('Args.env_id')['success rate (%)'].plot \
        .bar(yerr=df['±25%'] * 2, width=0.9, subplots=True, ylim=[0, 100], legend=False,
             color=colors, rot=15, figsize=(3, 3))
    for i in plt.gca().patches:
        # get_x pulls left or right; get_height pushes up or down
        plt.gca().text(i.get_x() + 0.1, i.get_height() - 15, str(round((i.get_height()), 2)), fontsize=11,
                       color='white', rotation=0, multialignment='center')
...
def _(sweep_root):
    ...
    # collect_data(groupby=['Args.env_id'])
    make_bar_chart()
...
if __name__ == "__main__":
    ...
    logger.upload_file("analysis.charts.yml", "img_maze_local_metric.charts.yml")
    jaynes.config('local')
    jaynes.run(_, sweep_root=Config.sweep_root)
    jaynes.listen()
