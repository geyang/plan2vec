...
def plan2vec():
    ...
    for deps in sweep:
        thunk = instr(main, deps, __postfix=f"MlpGlobalMetric/{Args.env_id}/plan_steps-({Args.plan_steps})")
        config_charts()
        jaynes.run(thunk)
...
def plan2vec_random_policy():
    ...
    for deps in sweep:
        thunk = instr(main, deps, __postfix=f"random_policy/{Args.env_id}/plan_steps-({Args.plan_steps})")
...
def sptm_baseline():
    ...
    for deps in sweep:
        thunk = instr(main, deps, __postfix=f"sptm-plan/{Args.env_id}/plan_steps-({Args.plan_steps})")
        config_charts(path="oracle_planner.charts.yml")
        logger.log_text("""
        # SPTM baseline:
