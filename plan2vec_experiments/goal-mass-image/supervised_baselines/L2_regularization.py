"""
To compare the effect of L2 regularization in fighting decay in the value function.
"""
...
def common_config():
    ...
    Args.num_epochs = 200_000
    Args.visualization_interval = 10

    # DEBUG.global_pretrain = True
...
if __name__ == "__main__":
    ...
    config_dash("""
    charts:
    - {glob: "figures/**/embed_2d*.png", type: file}
    - {xKey: epoch, yKey: "success_rate/mean", yDomain: [0, 1]}
    keys:
    - run.status
    - Args.weight_decay
    - Args.global_metric
    ...
    for Args.global_metric in ['ResNet18L2', 'ResNet18CoordL2', 'GlobalMetricCoordConvL2', 'GlobalMetricConvL2_s1']:
        for Args.lr in [8e-4, 1e-3, 3e-3, 1e-2, ]:
            for Args.weight_decay in [0, 5e-7, 1e-6, 5e-6]:
