...
@cli_parse
class Config:
    sweep_root = "geyang/plan2vec/2019/07-22/streetlearn/supervised_value_fn"
...
def collect_data(no_cache=False, include=None, exclude=("run*", "args", "kwargs*", "host*"), predicate=None,
    ...
    with logger.PrefixContext(Config.sweep_root):
        ...
        for run_prefix, params in tqdm(df.iterrows(), total=len(df.index), desc="loading parameter"):
            _ = single_run_stats("/" + run_prefix, "epoch", "success_rate", x_bin_size=100)
            if _ is None:
                continue
            # idx = _['mean'].idxmax()
...
if __name__ == "__main__":
    ...
    sweep_roots = [
        "geyang/plan2vec/2019/07-22/streetlearn/supervised_value_fn"
    ]

    import jaynes

    jaynes.config('devfair', runner=dict(n_cpu=10))
    # jaynes.config('local')
