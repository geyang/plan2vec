...
def plan2vec_coord_pretrain(dataset, prefix):
    ...
    _ = instr(main, __postfix=f"coord-pretrain/{prefix}", **vars(Args), _DEBUG=vars(DEBUG))
...
def plan2vec_value_pretrain(dataset, prefix):
    ...
    DEBUG.ground_truth_neighbor_r = 2e-4
    DEBUG.real_r_distance = False
    _ = instr(main, __postfix=f"value-pretrain/{prefix}", **vars(Args), _DEBUG=vars(DEBUG))
    config_charts(path="value-pretrain.charts.yml")
    jaynes.run(_)


def plan2vec_value_n_step_pretrain(dataset, prefix, n: int = 10):
    Args.data_path = f"~/fair/streetlearn/processed-data/{dataset}"
    ...
    DEBUG.supervised_value_fn = True
    DEBUG.value_fn_pretrain_goal_r = 1.2e-4 * float(n)
    ...
    DEBUG.ground_truth_neighbor_r = 2e-4
    DEBUG.real_r_distance = False
    _ = instr(main, __postfix=f"value-{n:02d}-step-pretrain/{prefix}", **vars(Args), _DEBUG=vars(DEBUG))
    config_charts(path="value-pretrain.charts.yml")
    jaynes.run(_)
...
def plan2vec_coord_value_pretrain(dataset, prefix):
    ...
    DEBUG.ground_truth_neighbor_r = 2e-4
    DEBUG.real_r_distance = False
    _ = instr(main, __postfix=f"coord-value-pretrain/{prefix}", **vars(Args), _DEBUG=vars(DEBUG))
...
if __name__ == "__main__":
    ...
    for key in ['tiny', 'small', 'medium', 'large', 'xl']:
        for lr in _['lr']:
            ...
            for Args.lr in lr / np.array([10, 4, 1]):
                # plan2vec_value_pretrain(f"manhattan-{key}",
                #                         f"dim-({Args.latent_dim})/manhattan-{key}/{Args.global_metric}/lr-({Args.lr})")
                plan2vec_value_n_step_pretrain(f"manhattan-{key}",
                                               f"dim-({Args.latent_dim})/manhattan-{key}/{Args.global_metric}/lr-({Args.lr})",
                                               n=4)

            # jaynes.listen()
            # plan2vec_coord_pretrain(f"manhattan-{key}",
            #                         f"manhattan-{key}/{Args.global_metric}/lr-({DEBUG.pretrain_lr})")
            # plan2vec_value_pretrain(f"manhattan-{key}",
            #                         f"manhattan-{key}/{Args.global_metric}/lr-({DEBUG.pretrain_lr})")
